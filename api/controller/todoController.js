...
exports.search_a_item = async (req, res) => {
    try {
        const { id, name } = req.query
        const data = await Task.find({ _id: id })
        const dataName = await Task.find({ name: { $regex: name, $options: 'i' } })
        const length = dataName.length - 1
        const idName = dataName[length].id
        res.json({
            message: 'thành công',
            data,
            dataName,
